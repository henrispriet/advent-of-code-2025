...
partitionMove pos move =
    let
        remainder = remainderTillZero pos move
    in
    if abs move <= abs remainder then
        (move, 0)
    else
...
updateStateInParts state move =
    let pos = snd state in
    let (part, remainder) = partitionMove pos move in
    let newState = updateState' state part in
